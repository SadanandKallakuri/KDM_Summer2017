...
object SparkNaiveBayes {
  ...
  private def run(params: Params) {
    ...
    val conf = new SparkConf().setAppName(s"NBExample with $params").setMaster("local[*]").set("spark.driver.memory", "4g").set("spark.executor.memory", "6g")
    val sc = new SparkContext(conf)

    Logger.getRootLogger.setLevel(Level.WARN)

    val topic_output = new PrintStream("data/NB_Results_TFIDF_YQA.txt")
    ...
    val IMAGE_CATEGORIES = List("BusinessandFinance", "ComputersandInternet", "EducationandReference")
    var index = 0
    ...
    val splits = featureVector.randomSplit(Array(0.8, 0.2), seed = 11L)
  ...
  private def preprocess(sc: SparkContext, paths: Seq[String]): (RDD[Vector], RDD[(String, String)], Long) = {
    ...
    val dff = sc.wholeTextFiles(paths.mkString(","), 1000)
    val df = dff.map(f => {
    ...
    val dff1 = stopWordRemovedDF.flatMap(f => f._2)
    val vocab = dff1.distinct().collect()
    (tfidf, data, dff1.count()) // Vector, Data, total token count
  }
